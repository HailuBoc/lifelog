...
export default function SignUpPage() {
  ...
  const debouncedConfirm = useDebounce(confirm, 200);
  ...
  useEffect(() => {
    if (debouncedEmail || debouncedPassword || debouncedConfirm) {
      validate();
    }
  }, [debouncedEmail, debouncedPassword, debouncedConfirm, validate]);
  ...
  }, [name, email, password, router, validate, announce]);

  const strength = useMemo(() => passwordStrength(password), [password]);
  const strengthLabel = useMemo(() => 
    ["Very weak", "Weak", "Okay", "Strong", "Very strong"][strength] || "Very weak",
    [strength]
  );
  const strengthPct = useMemo(() => (strength / 4) * 100, [strength]);
      ...
      >
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="flex-1 px-4 py-2 rounded-md bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-medium focus:outline-none focus:ring-2 focus:ring-indigo-500 disabled:opacity-60 transition-all duration-200 active:scale-95 flex items-center justify-center gap-2"
          >
            {saving || isLoading ? (
              <>
